...
import { Lightbulb, Loader2, Trophy, CalendarDays, Download, Globe, SlidersHorizontal } from "lucide-react";
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <TabsList className="grid w-full grid-cols-3 md:inline-flex md:w-auto">
                    <TabsTrigger value="current">
                        <CalendarDays className="size-5 md:mr-2" />
                        <span className="hidden md:inline">This Month</span>
                    </TabsTrigger>
                    <TabsTrigger value="all">
                        <Globe className="size-5 md:mr-2" />
                        <span className="hidden md:inline">All Time</span>
                    </TabsTrigger>
                    <TabsTrigger value="custom">
                        <SlidersHorizontal className="size-5 md:mr-2" />
      ...
      <Card>
        ...
        <CardContent>
            <div className="flex flex-col sm:flex-row items-center gap-4">
                <Button onClick={handleDownloadPDF} disabled={isLoading || isDownloading || periodTransactions.length === 0} className="w-full sm:w-auto flex-shrink-0">
                    {isDownloading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
                    {isDownloading ? 'Generating PDF...' : 'Download Report'}
                </Button>
                <p className="text-sm text-muted-foreground truncate">
                    For period: <span className="font-medium">{periodLabel}</span>
                </p>
            </div>
        </CardContent>
      </Card>
